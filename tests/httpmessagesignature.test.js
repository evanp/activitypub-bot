...
describe('HTTPMessageSignature', async () => {
  ...
  it('can sign a GET request', async () => {
    ...
    assert.ok(result['signature-input'].match(/sig1=\("@method" "@target-uri"\);keyid="[^"]+";alg="rsa-v1_5-sha256";created=\d+/))
  ...
  it('can sign a POST request', async () => {
    ...
    const result = await httpMessageSignature.sign({ privateKey, keyId, url, method, headers })
    assert.ok(result)
    assert.ok(result['signature-input'].match(/sig1=\("@method" "@target-uri" "content-digest"\);keyid="[^"]+";alg="rsa-v1_5-sha256";created=\d+/))
    assert.ok(result.signature)
    assert.ok(result.signature.match(/^sig1=:.+:$/))
  })
})
