...
export class BotContext {
  ...
  async unannounceObject (obj) {
    assert.ok(obj)
    assert.equal(typeof obj, 'object')
    const announceId = await this.#actorStorage.getLastActivity(
      this.#botId,
      'Announce',
      obj
    )
    ...
    const announceActivity = await this.#objectStorage.read(announceId)
    assert.ok(announceActivity)
    const recipients = this.#getRecipients(announceActivity)
    assert.ok(recipients)
    return await this.#doActivity({
      type: 'Undo',
      summary: {
        en: `${this.#botId} undid ${announceActivity}`
      },
      object: {
        id: announceId,
        type: 'Announce'
