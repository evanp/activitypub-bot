...
export async function makeApp ({ databaseUrl, origin, bots, logLevel = 'silent', deliveryWorkerCount = 2, distributionWorkerCount = 8, fanoutWorkerCount = 4, indexFileName = DEFAULT_INDEX_FILENAME, profileFileName = DEFAULT_PROFILE_FILENAME }) {
  const logger = Logger({
    level: logLevel
  })
  ...
  const fanoutWorkers = new Array(fanoutWorkerCount)
  const fanoutWorkerRuns = new Array(fanoutWorkerCount)

  for (let i = 0; i < fanoutWorkers.length; i++) {
    fanoutWorkers[i] = new FanoutWorker(
      jobQueue,
      distributor,
      logger
    )
    fanoutWorkerRuns[i] = fanoutWorkers[i].run().catch((err) => {
      logger.error(
        { err, workerIndex: i },
        'unexpected error in fanout worker')
    })
  }
