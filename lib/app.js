...
import { ActivityPubClientError } from './errors/activitypubclienterror.js'
...
import { DomainBlocker } from './domainblocker.js'
...
export async function makeApp ({ databaseUrl, origin, bots, logLevel = 'silent', deliveryWorkerCount = 2, distributionWorkerCount = 8, fanoutWorkerCount = 4, intakeWorkerCount = 2, indexFileName = DEFAULT_INDEX_FILENAME, profileFileName = DEFAULT_PROFILE_FILENAME, allowPrivateNetworkRequests = false, redisUrl, trustProxy, domainBlockFileName }) {
  const logger = Logger({
    level: logLevel
  })
  ...
  const domainBlocker = new DomainBlocker(domainBlockFileName, connection, logger)
  ...
  const fetcher = new SafeFetcher(domainBlocker, { allowPrivate: allowPrivateNetworkRequests })
  const client = new ActivityPubClient(keyStorage, formatter, signer, digester, logger, throttler, remoteObjectCache, messageSigner, policyStore, fetcher, domainBlocker)
  ...
  const authorizer = new Authorizer(actorStorage, formatter, client, domainBlocker)
