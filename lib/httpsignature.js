...
export class HTTPSignature {
  ...
  async validate (publicKeyPem, signature, method, path, headers) {
    ...
    this.debug({ signingString, signatureString, keyId, algorithm, path, host: headers.host, signedHeaders }, 'validating signature')
  ...
  async sign ({ privateKey, keyId, url, method, headers }) {
    ...
    this.debug({ keyId, url, method, headers, headersList, signingString, signature }, 'signed a request')
