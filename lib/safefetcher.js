import assert from 'node:assert'
import dns from 'node:dns'
import dnsPromises from 'node:dns/promises'
import https from 'node:https'

import fetch from 'node-fetch'
import ipaddr from 'ipaddr.js'

import { ProtocolError } from './errors/protocolerror.js'
import { PrivateNetworkError } from './errors/privatenetworkerror.js'
import { BlockedDomainError } from './errors/blockeddomainerror.js'

function isPrivateIP (address) {
  if (!ipaddr.isValid(address)) return false
  const addr = ipaddr.parse(address)
  const range = addr.range()
  return range !== 'unicast'
}

class SafeAgent extends https.Agent {
  constructor (options = {}) {
    super({
      keepAlive: true,
      keepAliveMsecs: 1000,
      maxSockets: 64,
      maxFreeSockets: 256,
      ...options
    })
  }

  createConnection (options, callback) {
    dns.lookup(options.hostname, (err, address) => {
      if (err) {
        return callback(err)
      }
      if (isPrivateIP(address)) {
        return callback(new PrivateNetworkError(address))
      }
      try {
        const socket = super.createConnection({
          ...options,
          host: address,
          servername: options.servername || options.hostname
        })
        callback(null, socket)
      } catch (e) {
        callback(e)
      }
    })
  }
}

export class SafeFetcher {
  #allowPrivate
  #agent
  #domainBlocker

  constructor (domainBlocker, options = {}) {
    assert.strictEqual(typeof domainBlocker, 'object')
    this.#domainBlocker = domainBlocker
    const { allowPrivate } = options
    this.#allowPrivate = !!allowPrivate
    this.#agent = (this.#allowPrivate)
      ? null
      : new SafeAgent()
  }

  get allowPrivateNetworkRequests () {
    return this.#allowPrivate
  }

  async fetch (url, options = {}) {
    if (!(await this.#checkProtocol(url))) {
      throw new ProtocolError(url)
    }
    if (await this.#domainBlocker.isBlocked(url)) {
      throw new BlockedDomainError('blocked domain', url, options.method || 'GET')
    }
    const fullOptions = {
      ...options,
      agent: this.#agent ?? undefined,
      signal: options?.signal ?? AbortSignal.timeout(10000),
      size: 1024 * 1024,
      follow: 10
    }
    return await fetch(url, fullOptions)
  }

  async #checkProtocol (url) {
    const parsed = (new URL(url))
    switch (parsed.protocol) {
      case 'https:':
        return true
      case 'http:': {
        if (this.#agent) {
          return false
        }
        const { address } = await dnsPromises.lookup(parsed.hostname)
        const addr = ipaddr.parse(address)
        const range = addr.range()
        return range !== 'unicast'
      }
      default:
        return false
    }
  }
}
