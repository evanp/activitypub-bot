...
import { DuplicateDeliveryError } from '../errors/duplicatedeliveryerror.js'
import { PrincipalActorMismatchError } from '../errors/principalactormismatcherror.js'
import { ActorNotAuthorizedError } from '../errors/actornotauthorizederror.js'
import { PrincipalNotAuthorizedError } from '../errors/principalnotauthorizederror.js'

const router = express.Router()

router.post('/user/:username/inbox', async (req, res, next) => {
  const { username } = req.params
  const { bots, deliverer, actorStorage, domainBlocker } = req.app.locals
  ...
  if (await domainBlocker.isBlockedObject(activity)) {
    logger.warn(
      { reqId: req.id, activity: activity.id, subject },
      'Silently dropping activity with blocked domain'
    )
    res.status(202)
    res.type('text/plain')
