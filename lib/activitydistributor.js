...
export class ActivityDistributor {
  ...
  constructor (client, formatter, actorStorage, logger, jobQueue, cache, domainBlocker) {
    assert.ok(client)
    assert.ok(formatter)
  ...
  async fanout (activity, username) {
    ...
    for await (const recipient of this.#public(activity, username)) {
      ...
      if (await this.#isLocal(recipient)) {
        ...
        if (await this.#domainBlocker.isBlocked(recipient)) {
          this.#logger.warn(
            { activity: activity.id, recipient },
            'Skipping delivery to blocked recipient'
          )
          continue
        }
        const inbox = await this.#getInbox(recipient, username)
        if (!inbox) {
          this.#logger.warn({ recipient }, 'No inbox')
        } else if (await this.#domainBlocker.isBlocked(inbox)) {
          this.#logger.warn(
            { activity: activity.id, recipient, inbox },
            'Skipping delivery to blocked inbox'
          )
    ...
    for await (const recipient of this.#private(activity, username)) {
      ...
      if (await this.#isLocal(recipient)) {
        ...
        if (!inbox) {
          this.#logger.warn({ recipient }, 'No direct inbox')
        } else if (await this.#domainBlocker.isBlocked(inbox)) {
          this.#logger.warn(
            { activity: activity.id, recipient, inbox },
            'Skipping delivery to blocked inbox'
