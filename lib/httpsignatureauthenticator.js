...
export class HTTPSignatureAuthenticator {
  ...
  async authenticate (req, res, next) {
    ...
    try {
      ...
      let result = await this.#signer.validate(publicKeyPem, signature, method, path, headers)
      ...
      if (!result) {
        ...
        if (ok2.publicKeyPem === ok.publicKeyPem) {
          ...
          result = await this.#signer.validate(publicKeyPem, signature, method, path, headers)
